...
OKUNA_CLI_CONFIG_FILE = os.path.join(current_dir, '.siuu-cli.json')
OKUNA_CLI_CONFIG_FILE_TEMPLATE = os.path.join(current_dir, 'templates/.siuu-cli.json')
...
def _get_mysql_password():
    return "123456789" #_get_random_string(64)


def _get_redis_password():
    return "123456789" #_get_random_string(128)
...
def _check_siuu_api_is_running(address, port):
    # Create a TCP socket
    try:
...
def _wait_until_api_is_running(address, port, message='Waiting for server to come up...', sleep=None):
    ...
    is_running = _check_siuu_api_is_running(address=address, port=port)
...
def _clean():
    """
    Cleans everything that the siuu-cli has created. Docker volumes, config files, everything.
    :return:
    """
    logger.info('🧹 Cleaning up database')
    subprocess.run(["docker", "volume", "rm", "siuu-api_mariadb"])
    subprocess.run(["docker", "volume", "rm", "siuu-api_redisdb"])
...
def _ensure_has_local_api_environment_file(siuu_cli_config):
    ...
    _replace_in_file(LOCAL_API_ENV_FILE, {
        "{{DJANGO_SECRET_KEY}}": siuu_cli_config['djangoSecretKey'],
        "{{SQL_PASSWORD}}": siuu_cli_config['sqlPassword'],
        "{{REDIS_PASSWORD}}": siuu_cli_config['redisPassword'],
    })


def _ensure_has_docker_compose_api_environment_file(siuu_cli_config):
...
def _ensure_has_siuu_config_file():
...
def _bootstrap(is_local_api):
    logger.info('🚀 Bootstrapping Siuu with some data')
...
def _ensure_has_required_cli_config_files():
    _ensure_has_siuu_config_file()
    with open(OKUNA_CLI_CONFIG_FILE, 'r+') as siuu_cli_config_file:
        siuu_cli_config = json.load(siuu_cli_config_file)
        _ensure_has_docker_compose_api_environment_file(siuu_cli_config=siuu_cli_config)
        _ensure_has_local_api_environment_file(siuu_cli_config=siuu_cli_config)


def _ensure_was_bootstrapped(is_local_api):
    with open(OKUNA_CLI_CONFIG_FILE, 'r+') as siuu_cli_config_file:
        siuu_cli_config = json.load(siuu_cli_config_file)
        if siuu_cli_config['bootstrapped']:
            return

        logger.info('Siuu was not bootstrapped.')

        _bootstrap(is_local_api=is_local_api)

        siuu_cli_config['bootstrapped'] = True
        siuu_cli_config_file.seek(0)
        json.dump(siuu_cli_config, siuu_cli_config_file, indent=4)
        siuu_cli_config_file.truncate()

        logger.info('Siuu was bootstrapped.')
...
@click.command()
def up_full():
    """Bring the whole of Siuu up"""
    _print_siuu_logo()
    _ensure_has_required_cli_config_files()
    _copy_requirements_txt_to_docker_images_dir()

    logger.info('⬆️  Bringing the whole of Siuu up...')

    atexit.register(_down_full)
    subprocess.run(["docker-compose", "-f", "docker-compose-full.yml", "up", "-d", "-V"])

    siuu_api_address = '127.0.0.1'
    siuu_api_port = 80

    _wait_until_api_is_running(address=siuu_api_address, port=siuu_api_port)

    _ensure_was_bootstrapped(is_local_api=False)

    logger.info('🥳  Siuu is live at http://%s:%s.' % (siuu_api_address, siuu_api_port))
...
def up_services_only():
    """Bring only the Siuu services up. API is up to you."""
    _print_siuu_logo()
    _ensure_has_required_cli_config_files()
    _copy_requirements_txt_to_docker_images_dir()

    logger.info('⬆️  Bringing only the Siuu services up...')
    ...
    logger.info('🥳  Siuu services are up')
...
@click.command()
def up_test():
    """Bring the Siuu test services up"""
    _print_siuu_logo()
    _ensure_has_required_cli_config_files()

    logger.info('⬆️  Bringing the Siuu test services up...')

    atexit.register(_down_test)
    subprocess.run(["docker-compose", "-f", "docker-compose-test-services-only.yml", "up", "-d", "-V"])

    logger.info('🥳  Siuu  tests services are live')
...
@click.command()
def build_full():
    """Rebuild Siuu services"""
    _ensure_has_required_cli_config_files()
    logger.info('👷‍♀️  Rebuilding Siuu full services...')
    _copy_requirements_txt_to_docker_images_dir()
...
def build_services_only():
    """Rebuild Siuu services"""
    _ensure_has_required_cli_config_files()
    logger.info('👷‍♀️  Rebuilding only Siuu services...')
    _copy_requirements_txt_to_docker_images_dir()
